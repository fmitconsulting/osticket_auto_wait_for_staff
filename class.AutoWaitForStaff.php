 ...
 */
class AutoWaitForStaffPlugin extends Plugin {
	const DEBUG = FALSE;
	/**
	 ...
	 * @var string
	 */
	public $config_class = 'AutoWaitForStaffPluginConfig';
	...
	function bootstrap() {
		Signal::connect ( 'threadentry.created', function (ThreadEntry $entry) {
			...
			if ($entry->getUserId ()) {
				...
				//$ticket->setStatusId(1); // this id dirty hack that assumes that desired new status has id '1', just for testing
				//$ost->logDebug("ThreadEntry detected.", sprintf('getDefaultTicketStatusId = %d', $cfg->getDefaultTicketStatusId()), true);
				$ticket->setStatusId($cfg->getDefaultTicketStatusId()); // here we assume that default ticket status is "waiting for staff"
			}
		} );
	}
